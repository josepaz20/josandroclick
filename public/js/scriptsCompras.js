...
function gestionaraprobaciones(idItem) {
    $.get('gestionaprobaciones', {idItem: idItem}, setFormulario);
    bloqueoAjax();
}

function verGestionCotizacion(idItem) {
    $.get('gestioncotizaciones', {idItem: idItem}, setFormulario);
    bloqueoAjax();
}
...
function verPagarItem(idItem) {
    $.get('verpagoitem', {idItem: idItem}, setFormulario);
    bloqueoAjax();
}
...
function registraritems() {
    ...
    fila += " <td><input type='number' id='cantidad_" + contItems + "' name='cantidad_" + contItems + "' class='form-control' style='width: 85px' required></td>";
    fila += " <td><input type='number' id='vlrunitario_" + contItems + "' name='vlrunitario_" + contItems + "' class='form-control' style='width: 125px'></td>";
...
function validarAddAprobacion() {
    if($("#idItem").val() == 0 || $("#idItem").val() == '' || $("#idItem").val() == '{idItem}' || $("#aprobadopor").val() == '') {
    ...
    if (confirm("   DESEA REGISTRAR SU APROBACION PARA ESTE ITEM ? ")) {
        bloqueoAjax();
        return true;
    } else {
        return false;
